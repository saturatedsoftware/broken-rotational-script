...
public class TEST_ROTATION_FOR_PUZZLE : Puzzle_Interface_Targetable_Base_Class, Ipuzzle
{
    ...
    void OnMouseDrag()
    {
        ...
        if (HUD_manager.CurrentMenuState == HUD_manager.MenuState.PuzzleMenu)   //WAS   gameController_Master.isPuzzleMenuActive
        {
            ...
            {
                transform.localRotation = Quaternion.AngleAxis(mouseLook.y + offsetRotation, Vector3.right);    //up/down
            }
            if (currentRotationType == rotationType.Horizontal)
            {
                transform.localRotation = Quaternion.AngleAxis(mouseLook.x + offsetRotation, Vector3.up);    //right/left
            }
